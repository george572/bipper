...
const showTakeProductsPage = ref(false);
const showAddProductsPage = ref(false);
onMounted(() => {
    if(route && route.query && route.query.action) {
        if (route.query.action === 'take') {
            showTakeProductsPage.value = true;
        } else {
            showAddProductsPage.value = true;
        }
    }
})
</script>
